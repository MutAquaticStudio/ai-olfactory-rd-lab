...
export default function ProbabilityBars({ items, limit, showChemesthesis = false, hideZero = false }: {
      ...
      {visibleItems.slice(0, limit).map((item) => {
        ...
        return (
          <div className="probability-row" key={item.name}>
            <div className="probability-label"><span>{item.name}</span>{showChemesthesis && chemesthesis.has(item.name) && <small>{copy.chemesthesis}</small>}<strong>{percent.toFixed(1)}%</strong></div>
